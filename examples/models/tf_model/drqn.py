...
class DeepRecurrentQNetwork(TFBaseModel):
    def __init__(
        ...
        self.input_view = tf.placeholder(
            tf.float32, (None,) + self.view_space, name="input_view"
        )
        self.input_feature = tf.placeholder(
            tf.float32, (None,) + self.feature_space, name="input_feature"
        )
        ...
        with tf.variable_scope(self.name):
            with tf.variable_scope("eval_net_scope"):
                ...
                self.qvalues, self.state_in, self.rnn_state = self._create_network(
                    self.input_view, self.input_feature
                )
            ...
            with tf.variable_scope("target_net_scope"):
                ...
                (
                    self.target_qvalues,
                    self.target_state_in,
                    self.target_rnn_state,
        ...
        self.td_error = tf.square(
            self.target
            - tf.reduce_sum(tf.multiply(self.actions_onehot, self.qvalues), axis=1)
        )
        ...
        t_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, self.target_scope_name
        )
        e_params = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, self.eval_scope_name
        )
    ...
    def _create_network(self, input_view, input_feature, reuse=None):
        ...
        flatten_view = tf.reshape(
            h_conv2, [-1, np.prod([v.value for v in h_conv2.shape[1:]])]
        )
        ...
        rnn_in = tf.reshape(
            dense, shape=[self.batch_size_ph, self.unroll_step_ph, state_size]
        )
    ...
    def infer_action(self, raw_obs, ids, policy="e_greedy", eps=0):
        ...
        qvalues, states = self.sess.run(
            [self.qvalues, self.rnn_state],
            feed_dict={
    ...
    def _calc_target(
        self, next_view, next_feature, rewards, terminal, batch_size, unroll_step
    ):
        """calculate target value"""
        n = len(rewards)
        if self.use_double:
            t_qvalues, qvalues = self.sess.run(
                [self.target_qvalues, self.qvalues],
                feed_dict={
    ...
    def train(self, sample_buffer, print_every=500):
        ...
        n_batches = (
            self.train_freq * add_num / (batch_size * (unroll_step - self.skip_error))
        )
        if n_batches == 0:
            return 0, 0
        ...
        print(
            "batches: %d  add: %d  replay_len: %d/%d"
        ...
        for i in range(n_batches):
            ...
            if ct % print_every == 0:
                print(
                    "batch %5d, loss %.6f, qvalue %.6f" % (ct, loss, self._eval(target))
                )
            ct += 1
            self.train_ct += 1
        ...
        print(
            "batches: %d,  total time: %.2f,  1k average: %.2f"
            % (ct, total_time, step_average)
        )
    ...
    def train_keep_hidden(self, sample_buffer, print_every=500):
        ...
        print(
            "batches: %d  add: %d  replay_len: %d, %d/%d"
            % (
                n_batches,
                add_num,
                replay_lens_sum,
        ...
        while ct < n_batches:
            ...
            for j in range((train_length + unroll_step - 1) / unroll_step):
                ...
                next_pick = np.empty(
                    (n_rows, unroll_step + 1), dtype=np.int32
                )  # next pick choose one more state than pick
                ...
                batch_target = self._calc_target(
                    batch_view[next_pick],
                    batch_feature[next_pick],
                    batch_reward[pick],
                    batch_terminal[pick],
                    batch_hidden[:n_rows],
                    n_rows,
                ...
                if ct % print_every == 0:
                    print(
                        "batches %5d, mask %d/%d (%d), loss %.6f, qvalue %.6f"
                        % (
                            ct,
                            sum(batch_mask),
                            n_rows * train_length,
