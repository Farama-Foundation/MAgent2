...
class TFBaseModel(BaseModel):
    ...
    def save(self, dir_name, epoch):
        ...
        saver.save(
            self.sess, os.path.join(dir_name, (self.subclass_name + "_%d") % epoch)
        )
    ...
    def load(self, dir_name, epoch=0, name=None):
        ...
        if (
        ...
        ):  # the name of saved model is the same as ours
            dir_name = os.path.join(dir_name, self.name)
            model_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, self.name)
            saver = tf.train.Saver(model_vars)
            saver.restore(
            ...
            with tf.Graph().as_default(), tf.Session() as sess:
                tf.train.import_meta_graph(
                    os.path.join(
                        dir_name, name, (self.subclass_name + "_%d") % epoch + ".meta"
                    )
                )
                ...
                saver = tf.train.Saver(model_vars)
                saver.restore(
                    sess, os.path.join(dir_name, (self.subclass_name + "_%d") % epoch)
                )
