...
class DeepQNetwork(TFBaseModel):
    def __init__(
        ...
        self.infer_batch_size = (
            infer_batch_size  # maximum batch size when infer actions,
        )
        ...
        with tf.variable_scope(self.name):
            with tf.variable_scope("eval_net_scope"):
                ...
                self.qvalues = self._create_network(
                    self.input_view, self.input_feature, use_conv
                )
        ...
        def out_action(qvalues):
            ...
            random_action = tf.random_uniform(
                tf.shape(best_action), 0, self.num_actions, tf.int32
            )
        ...
        self.replay_buf_feature = ReplayBuffer(
            shape=(memory_size,) + self.feature_space
        )
    ...
    def _create_network(self, input_view, input_feature, use_conv=True, reuse=None):
        ...
        if self.use_dueling:
            ...
            qvalues = tf.layers.dense(
                dense, units=self.num_actions, name="value", reuse=reuse
            )
    ...
    def _calc_target(self, next_view, next_feature, rewards, terminal):
        ...
        if self.use_double:
            ...
            t_qvalues = self.sess.run(
                self.target_qvalues,
                {self.input_view: next_view, self.input_feature: next_feature},
            )
    ...
    def train(self, sample_buffer, print_every=1000):
        ...
        print(
            "batch number: %d  add: %d  replay_len: %d/%d"
        ...
        for i in range(n_batches):
            ...
            batch_target = self._calc_target(
                batch_next_view, batch_next_feature, batch_reward, batch_terminal
            )
            ...
            if ct % print_every == 0:
                print(
                    "batch %5d,  loss %.6f, eval %.6f"
                    % (ct, loss, self._eval(batch_target))
                )
            ct += 1
            self.train_ct += 1
    ...
    def _build_multi_gpu_infer(self, num_gpu):
        ...
        for i in range(num_gpu):
            self.infer_input_view.append(
                tf.placeholder(tf.float32, (None,) + self.view_space)
            )
            self.infer_input_feature.append(
                tf.placeholder(tf.float32, (None,) + self.feature_space)
            )
            with tf.variable_scope("eval_net_scope"), tf.device("/gpu:%d" % i):
                self.infer_qvalues.append(
                    self._create_network(
                        self.infer_input_view[i],
                        self.infer_input_feature[i],
                        reuse=True,
                    )
                )
