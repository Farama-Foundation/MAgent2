...
class DeepQNetwork(MXBaseModel):
    def __init__(
        ...
        self.model = mx.mod.Module(
            ...
            label_names=["action", "target", "mask"],
            context=self.ctx,
        )

        self.target_model = mx.mod.Module(
            self.qvalues,
            data_names=["input_view", "input_feature"],
            label_names=[],
            context=self.ctx,
        ...
        self.model.init_optimizer(
            optimizer="adam",
            optimizer_params={"learning_rate": learning_rate, "clip_gradient": 10.0},
        )
    ...
    def _create_network(self, input_view, input_feature, use_conv=True):
        ...
        if use_conv:
            ...
            h_conv1 = mx.sym.Convolution(
                data=input_view, kernel=(3, 3), num_filter=kernel_num[0], layout="NCHW"
            )
            h_conv1 = mx.sym.Activation(data=h_conv1, act_type="relu")
            h_conv2 = mx.sym.Convolution(
                data=h_conv1, kernel=(3, 3), num_filter=kernel_num[1], layout="NCHW"
            )
    ...
    def _calc_target(self, next_view, next_feature, rewards, terminal):
        ...
        data_batch = mx.io.DataBatch(
            data=[mx.nd.array(next_view), mx.nd.array(next_feature)]
        )
    ...
    def train(self, sample_buffer, print_every=1000):
        ...
        for i in range(n_batches):
            ...
            batch = mx.io.DataBatch(
                data=[mx.nd.array(batch_view), mx.nd.array(batch_feature)],
                label=[
                    mx.nd.array(batch_action),
    ...
    def _eval(self, target):
        ...
        if self.eval_obs is None:
            ...
            with self.ctx:
                batch = mx.io.DataBatch(
                    data=[mx.nd.array(self.eval_obs[0]), mx.nd.array(self.eval_obs[1])]
                )
                self.model.forward(batch, is_train=False)
