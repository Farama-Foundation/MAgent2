...
def play_a_round(
    ...
    while not done:
        ...
        if step_ct % print_every == 0:
            print(
                "step %3d,  deer: %5d,  tiger: %5d,  train_id: %d,  reward: %.2f,  total_reward: %.2f "
...
if __name__ == "__main__":
    ...
    # init the game
    env = magent2.GridWorld("double_attack", map_size=args.map_size)
    ...
    for k in range(start_from, start_from + args.n_round):
        ...
        loss, reward, value = play_a_round(
            env,
            args.map_size,
            [deer_handle, tiger_handle],
            models,
        ...
        log.info(
            "round %d\t loss: %s\t reward: %s\t value: %s" % (k, loss, reward, value)
        )
        print(
