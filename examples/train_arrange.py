...
def create_naive_maze(pos, width, height, unit, font_area):
    ...
    for i in range(height):
        if i % 2 == 0:
            pos_set.extend(
                draw_split_line(pos[0], pos[1] + i * unit, width * unit, unit)
            )
            pos_set.extend(
                draw_split_line(
                    pos[0], pos[1] + font_area[1] + i * unit, width * unit, unit
                )
            )
            pos_set.extend(
                draw_split_line(
...
def generate_map(env, map_size, goal_handle, handles):
    ...
    pos = pos[
        np.random.choice(
            np.arange(len(pos)),
            int(circle_goal_num + alpha_goal_num * 1.25),
            replace=False,
        )
    ]
...
def play_a_round(
    ...
    while not done:
        ...
        if 1.0 * len(pos_reward_ct) / goal_num >= 0.99:
            ...
            for i in range(len(pos)):
                x, y = pos[i]
                rate = (
                    1 - 1.0 * ((x - center_x) ** 2 + (y - center_y) ** 2) / normalizer
                )
                delta = -rate * 4.5 - 0.5
                rewards[i] += delta
        ...
        if train_id != -1:
            ...
            sample_buffer.record_step(
                ids[train_id], obs[train_id], acts[train_id], rewards, alives
            )
        ...
        if step_ct % print_every == 0:
            print(
                "step %3d,  train %d,  num %s,  reward %.2f,  total_reward: %.2f, non_zero: %d"
                % (
                    step_ct,
                    train_id,
                    [goal_num] + nums,
    ...
    print(
        "steps: %d, new_rule: %d, total time: %.2f,  step average %.2f"
        % (step_ct, new_rule_ct, sample_time, sample_time / step_ct)
    )
    ...
    return (
        ...
        value,
        len(pos_reward_ct),
        1.0 * len(pos_reward_ct) / goal_num,
    )
...
if __name__ == "__main__":
    ...
    if args.record:
        ...
        for k in range(start_from, start_from + args.n_round):
            tic = time.time()
            eps = (
                buffer.piecewise_decay(k, [0, 400, 1200], [1.0, 0.2, 0.10])
                if not args.greedy
                else 0
            )
            loss, reward, value, pos_reward_ct, fill_rate = play_a_round(
                env,
                args.map_size,
                food_handle,
                player_handles,
