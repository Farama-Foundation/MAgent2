...
def extract_model_names(savedir, name, model_class, begin=0, pick_every=4):
    if model_class is DeepQNetwork:
        prefix = "tfdqn"
    pattern = re.compile(prefix + r"_(\d*).meta")
...
if __name__ == "__main__":
    ...
    # scan file names
    model_name = extract_model_names(
        "save_model", "battle", DeepQNetwork, begin=0, pick_every=5
    )
    ...
    def play_wrapper(model_names, n_rounds):
        ...
        for i, item in enumerate(model_names):
            models.append(ProcessingModel(env, handles[i], item[1], 0, item[-1]))
    ...
    for i in range(len(model_name)):
        for j in range(i + 1, len(model_name)):
            ...
            round_res = (
                "model1: %s\t model2: %s\t rate: %.2f\t num: %s\t elapsed: %.2f"
                % (
                    model_name[i][:-1],
                    model_name[j][:-1],
                    rate[i][j],
        ...
        winrate_file.write(
            "model: %s\twin rate: %.2f\n"
            % (model_name[i], 1.0 * sum(np.asarray(rate[i])) / (len(model_name) - 1))
        )
