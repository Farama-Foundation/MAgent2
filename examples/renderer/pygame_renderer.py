...
from examples.renderer import BaseRenderer
from examples.renderer.server import BaseServer
...
class PyGameRenderer(BaseRenderer):
    ...
    def start(  # noqa: C901
        ...
        def draw_rect_matrix(matrix, color, a, w, h, resolution):
            ...
            matrix[
                max(x, 0) : min(x + w, resolution[0]),
                max(y, 0) : min(h + y, resolution[1]),
                :,
            ] = color

        def draw_line_matrix(matrix, color, a, b, resolution):
            a = (
                min(max(0, a[0]), resolution[0] - 1),
                min(max(0, a[1]), resolution[1] - 1),
            )
            b = (
                min(max(0, b[0]), resolution[0] - 1),
                min(max(0, b[1]), resolution[1] - 1),
            )
        ...
        if full_screen:
            canvas = pygame.display.set_mode(
                resolution, pygame.DOUBLEBUF | pygame.FULLSCREEN, 0
            )
        else:
        ...
        view_position = [
            map_size[0] / 2 * grid_size - resolution[0] / 2,
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 4 or event.button == 5:
                        ...
                        if event.button == 5:
                            grid_size = max(
                                grid_size - grid_size * zoom_rate, grid_min_size
                            )
                            need_static_update = True
                        else:
                            grid_size = min(
                                grid_size + grid_size * zoom_rate, grid_max_size
                            )
                            need_static_update = True
                        center_after = (
                            (view_position[0] + resolution[0] / 2) / grid_size,
                        ...
                        triggered = server.mousedown(
                            frame_id, pygame.mouse.get_pressed(), mouse_x, mouse_y
                        )
            ...
            x_range = (
                ...
                min(
                    map_size[0],
                    int(
                        math.ceil(max(0, view_position[0] + resolution[0]) / grid_size)
                    ),
                ),
            )
            ...
            y_range = (
                ...
                min(
                    map_size[1],
                    int(
                        math.ceil(max(0, view_position[1] + resolution[1]) / grid_size)
                    ),
                ),
            )
            ...
            if (
                new_data is None
                or animation_progress > animation_total + animation_stop
            ):
                buffered_new_data = server.get_data(
                    frame_id,
                    (
                        view_position[0] / grid_size,
                        (view_position[0] + resolution[0]) / grid_size,
                    ),
                    (
                        view_position[1] / grid_size,
                        (view_position[1] + resolution[1]) / grid_size,
                    ),
                )
            ...
            if new_data is not None:
                ...
                for key in new_data[0]:
                    ...
                    old_prop = (
                        old_data[0][key]
                        if old_data is not None and key in old_data[0]
                        else None
                    )
                    new_group = groups[new_prop[2]]
                    old_group = groups[old_prop[2]] if old_prop is not None else None
                    now_prop = (
                        [a * (1 - rate) + b * rate for a, b in zip(old_prop, new_prop)]
                ...
                text_fps = text_formatter.render(
                    f"FPS: {int(clock.get_fps())}", True, text_rgb
                )
                ...
                text_grids = text_formatter.render(
                    "Numbers: %d" % len(new_data[0]), True, text_rgb
                )
                text_mouse = text_formatter.render(
                    "Mouse: (%d, %d)" % (mouse_x, mouse_y), True, text_rgb
                )
                ...
                if endscreen_texts:
                    ...
                    for texts in endscreen_texts:
                        ...
                        height = 0
                        for text in texts:
                            text = bigscreen_formatter.render(
                                text[0], True, pygame.Color(*text[1])
                            )
                            content.append([text])
                            height = max(height, text.get_height())
                    ...
                    for content in endscreen_contents:
                        ...
                        for b in content:
                            endscreen.blit(
                                ...
                                (
                                    width_now + b[1],
                                    resolution[1] / 2.0 - total_height + height,
                                ),
                            )
